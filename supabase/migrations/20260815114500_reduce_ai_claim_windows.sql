-- gpt-oss-120b에 reasoning_effort=low를 적용한 뒤 다시 측정한 결과,
-- 약 100,000자 채점은 8.9초, Day 3 퀴즈 stress case는 최대 16.7초에 완주했다.
-- 앱의 AI deadline(60초) < 페이지 maxDuration(90초) < 선점 만료(120초) 순서로
-- 맞춰 실패 후 재시도 대기 시간을 줄인다.
--
-- 이미 개발 DB에 적용된 20260812130000·20260813070000은 수정하지 않는다.
-- 두 함수의 시그니처와 반환형은 그대로이므로 새 마이그레이션에서 본문만 교체한다.
