-- 1:1 문의에 "관련 기능(area)" 축을 추가한다.
--
-- 기존 category는 유형(버그/제안/기타)만 구분해서, 관리자 화면에서 어느 기능의
-- 문제인지 알려면 본문을 열어봐야 했다. category는 그대로 두고 관련 기능을 area
-- 컬럼으로 분리해, 문의 유형 x 관련 기능 두 축으로 교차 필터할 수 있게 한다.
--
-- 기존 행은 관련 기능을 알 수 없으므로 default 'ETC'로 백필된다.
...
-- 관리자 목록은 관련 기능(area)을 multi-select 필터로 쓰고 created_at DESC로 정렬한다.
...
COMMENT ON COLUMN "public"."feedbacks"."area" IS
  '문의와 관련된 기능. 문의 유형(category)과 직교하는 축이다.';
