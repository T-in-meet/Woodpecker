ALTER TABLE public.notes DROP CONSTRAINT IF EXISTS notes_language_check;
ALTER TABLE public.notes DROP COLUMN IF EXISTS language;

DROP FUNCTION IF EXISTS public.create_note_with_initial_review_log(text, text, timestamptz, text);
...
BEGIN
  ...
  VALUES (v_user_id, p_title, p_content, p_scheduled_at)
