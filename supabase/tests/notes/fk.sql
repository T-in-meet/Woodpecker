...
SELECT set_config('test.note_seed_id', gen_random_uuid()::text, true);
SELECT set_config('test.note_insert_id', gen_random_uuid()::text, true);
SELECT set_config('test.note_stale_id', gen_random_uuid()::text, true);
...
VALUES
  (
    current_setting('test.note_seed_id')::uuid,
    current_setting('test.user_a_id')::uuid,
    'seed note',
    'seed content',
    0
...
SAVEPOINT notes_fk_insert_valid;
SELECT lives_ok(
  format(
    $sql$
...
ROLLBACK TO SAVEPOINT notes_fk_insert_valid;

-- auth.users에서 user_a를 삭제하면 user_a의 notes가 자동으로 삭제되어야 한다
SAVEPOINT notes_fk_delete_cascade;
...
ROLLBACK TO SAVEPOINT notes_fk_delete_cascade;
...
SELECT throws_ok(
  format(
    ...
    $sql$,
    current_setting('test.note_stale_id'),
...
SAVEPOINT notes_fk_stale_reference;
DELETE FROM auth.users WHERE id = current_setting('test.user_a_id')::uuid;
SELECT throws_ok(
  format(
    $sql$
...
ROLLBACK TO SAVEPOINT notes_fk_stale_reference;

-- user_a의 notes가 여러 개일 때 user_a를 삭제하면 모든 notes가 남김없이 삭제되어야 한다
SAVEPOINT notes_fk_delete_multiple;
INSERT INTO public.notes (id, user_id, title, content, review_round)
VALUES
  (
...
ROLLBACK TO SAVEPOINT notes_fk_delete_multiple;

-- user_a의 notes가 0개일 때 user_a를 삭제해도 오류 없이 성공해야 한다
SAVEPOINT notes_fk_delete_zero;
DELETE FROM public.notes WHERE user_id = current_setting('test.user_a_id')::uuid;
SELECT lives_ok(
  format(
    $sql$
...
ROLLBACK TO SAVEPOINT notes_fk_delete_zero;
