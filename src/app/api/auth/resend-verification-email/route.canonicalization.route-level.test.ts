...
describe("resend route canonical identity key contract", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("raw variant 2개(Test@Example.com / 공백 포함 test@example.com)는 동일 identity key로 처리된다", async () => {
    await postAfterMinimumTime("Test@Example.com", "10.1.0.2");
    await postAfterMinimumTime(" test@example.com ", "10.1.0.3");
