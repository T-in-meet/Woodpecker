...
describe("resend route canonicalization", () => {
  ...
  it("TC-01. Gmail plus alias는 canonical email로 전달된다", async () => {
  ...
  it("TC-02. Gmail dot alias는 canonical email로 전달된다", async () => {
    const response = await postAfterMinimumTime(
      "u.s.e.r@gmail.com",
      "10.0.0.2",
    );
  ...
  it("TC-03. googlemail.com은 gmail.com으로 canonicalize되어 전달된다", async () => {
    const response = await postAfterMinimumTime(
      "User@GoogleMail.com",
      "10.0.0.3",
    );
  ...
  it("TC-04. non-Gmail은 plus/dot을 보존한 canonical 값으로 전달된다", async () => {
    const response = await postAfterMinimumTime(
