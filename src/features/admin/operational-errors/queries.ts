...
export async function getOperationalErrorDetail(
  ...
  /** 운영 오류가 존재하지 않거나 조회에 실패하면 404 응답을 처리합니다. */
  if (error || !data) {
    notFound();
  }
  ...
  if (profileIds.length > 0) {
    ...
    /** 조회한 프로필의 ID와 닉네임을 빠르게 참조할 수 있도록 저장합니다. */
    for (const profile of (profiles ?? []) as ProfileLabelRow[]) {
      profileLabels.set(profile.id, profile.nickname);
    }
  }
