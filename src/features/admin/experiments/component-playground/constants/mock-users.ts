...
const USER_GRADES = [
  ...
  "premium",
  "vip",
] as const satisfies readonly MockUserGrade[];
...
/**
 * 사용자 순번에 따라 순환하는 등급 값을 반환합니다.
 */
function getMockUserGrade(index: number): MockUserGrade {
  return USER_GRADES[index % USER_GRADES.length] ?? USER_GRADES[0];
}

/**
 * 사용자 순번에 따라 순환하는 역할 목록을 반환합니다.
 *
 * readonly 튜플을 MockUser의 변경 가능한 배열 타입에 맞게 복사합니다.
 */
...
/**
 * 사용자 순번을 기준으로 서로 다른 생성일을 만듭니다.
 *
 * Date range 필터에서 다양한 날짜 범위를 확인할 수 있도록
 * 2025년 1월 1일부터 하루씩 증가하는 날짜를 사용합니다.
 */
