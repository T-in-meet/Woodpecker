...
/* 이 테스트가 지키는 건 화면 동작이 아니라 색인 가능성이다.
   Radix Accordion은 닫힌 content를 언마운트하므로 AccordionContent의
   forceMount가 빠지면 검색엔진과 JS를 실행하지 않는 AI 크롤러가 받는 HTML에서
   답변 전문이 사라진다. 열림/닫힘 표시는 CSS(data-state 기반 grid-rows·
   visibility 전환)가 맡으므로 여기서는 DOM에 남아 있는지와 data-state만
   확인한다. */
