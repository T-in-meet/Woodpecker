...
export const faqs = [
  ...
  // AI 채점 한도는 두 겹이다. 회차당 1번(review_gradings의 already_graded)과
  // 사용자당 하루 5번(claim_review_grading의 c_daily_limit, KST 자정 초기화).
  // 회차만 적으면 하루 6번째 요청에서 "오늘 받을 수 있는 AI 채점을 모두
  // 사용했어요"를 만나는 사용자에게 안내가 어긋나므로 둘 다 적는다.
  // 한도 값의 정본은 마이그레이션의 c_daily_limit이다. 그 값을 바꾸면 이 문구도
  // 함께 고친다.
  {
    question: "백지 테스트는 어떻게 진행되나요?",
    answer:
      "노트를 보지 않은 상태에서 기억나는 내용을 직접 작성한 뒤 원문과 비교하는 방식으로 진행됩니다. 필요하면 AI 채점을 요청해 빠뜨린 핵심 개념이나 원문과 다르게 기억한 부분을 확인할 수 있습니다. AI 채점은 복습 회차당 한 번 요청할 수 있고, 하루에 요청할 수 있는 횟수는 5번까지이며 매일 자정에 초기화됩니다. 결과는 학습을 돕기 위한 참고용으로 활용해 주세요.",
  },
        ...
        <Accordion type="single" collapsible className="mt-10">
          {faqs.map((faq, index) => (
            <AccordionItem key={faq.question} value={`item-${index}`}>
              ...
              {/* 접힌 답변도 HTML에 남는다(AccordionContent가 forceMount).
                  검색엔진과 JS를 실행하지 않는 AI 크롤러에게 이 페이지의 FAQ가
                  질문만 있고 답변은 없는 문서로 읽히지 않게 하기 위해서다.
                  JSON-LD의 FAQPage는 본문 콘텐츠를 대신하지 못한다. 구글은 접힌
                  UI 안의 콘텐츠도 HTML에 있으면 색인한다. */}
