...
export async function createOpenAiEmbedding(params: {
  ...
  // 외부 API 응답을 그대로 신뢰하지 않고 Provider 전용 스키마에서 검증한 뒤 사용한다.
  const parsedResponse = openAiEmbeddingResponseSchema.parse(
    (await response.json()) as unknown,
  );

  // OpenAI Embeddings API에는 출력 토큰이 없으므로 입력 토큰만 공통 usage 형식으로 정규화한다.
  ...
  // 빈 벡터가 이후 캐시 저장이나 유사도 검색 단계까지 전달되지 않도록 Provider 경계에서 차단한다.
  if (embedding.length === 0) {
    throw new Error("OpenAI embedding returned empty values.");
  }

  // 모델 설정과 실제 반환 벡터의 불일치를 DB 저장 이전에 감지하여 차원 오류를 조기에 노출한다.
