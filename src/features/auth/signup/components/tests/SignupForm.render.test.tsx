...
describe("회원가입 폼", () => {
  ...
  it("TC-03: 이용약관 및 개인정보 처리방침 내용을 확인할 수 있는 트리거를 렌더링한다", () => {
    renderSignupForm();
  ...
  it('TC-04: type="submit"인 회원가입 버튼을 렌더링한다', () => {
    renderSignupForm();
  ...
  it("TC-05: 모든 입력과 체크박스가 접근 가능한 label과 연결되어 있다", () => {
    renderSignupForm();
  ...
  it('TC-06: 비밀번호 및 비밀번호 확인 필드는 type="password"를 사용한다', () => {
    renderSignupForm();
  ...
  it('TC-07: 이메일 필드는 type="email"을 사용한다', () => {
    renderSignupForm();

    expect(screen.getByLabelText(/이메일/i)).toHaveAttribute("type", "email");
  });

  it("TC-08: 초기 렌더링 시 약관 체크박스는 체크되지 않은 상태이다", () => {
    renderSignupForm();
  ...
  it("TC-09: 초기 렌더링 시 회원가입 버튼은 활성 상태이다", () => {
    renderSignupForm();
