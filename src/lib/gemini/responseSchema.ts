...
/**
 * Gemini의 responseJsonSchema가 읽는 키워드 목록.
 * 문서에 명시된 것만 남기고 나머지는 버린다. 모르는 키를 그대로 보내면
 * 무시되는지 거부되는지 알 수 없어서, 통과가 보장된 것만 넘긴다.
 */
...
function sanitize(node: unknown): unknown {
  ...
  for (const [key, value] of Object.entries(node)) {
    // Gemini는 const를 읽지 못한다. 퀴즈 유형 고정이 이 스키마의 핵심이라 enum으로 바꾼다.
    if (key === "const") {
      result.enum = [value];
      continue;
    }
...
/**
 * Zod 스키마를 Gemini `responseJsonSchema` 값으로 바꾼다.
