...
describe("api response contract", () => {
  it("successResponse: success/code/data envelope과 기본 status 매핑을 보장한다", async () => {
    ...
    const response = successResponse(code, { redirectTo: ROUTES.RESEND_EMAIL });
    ...
    expect(body).toEqual({
      success: true,
      code: "SIGNUP_SUCCESS",
      data: { redirectTo: ROUTES.RESEND_EMAIL },
    });
  });
  ...
  it("message는 선택 필드이며 contract 핵심(success/code/data/status)과 독립적으로 동작한다", async () => {
    const response = successResponse(
      makeApiCode("signup", API_RESULTS.SUCCESS),
      { redirectTo: ROUTES.RESEND_EMAIL },
    ...
    expect(body.data).toEqual({ redirectTo: ROUTES.RESEND_EMAIL });
