...
큐를 사용하지 않는 브랜치는 **Supabase SQL Test**를 필수 상태 검사로 지정하고 **Require branches to be up to date before merging**을 활성화해야 합니다. 이 조건을 충족하면 이 검사를 위해 큐를 새로 활성화할 필요는 없습니다. base 브랜치 갱신만으로는 기존 PR의 `pull_request` CI가 재실행되지 않으므로, 최신 브랜치 조건이 없으면 앞선 PR 병합 전에 통과한 오래된 검사 결과로 순서가 뒤집힌 마이그레이션을 병합할 수 있습니다.
